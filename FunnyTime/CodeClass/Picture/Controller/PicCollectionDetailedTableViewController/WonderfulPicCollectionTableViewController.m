//
//  WonderfulPicCollectionTableViewController.m
//  FunnyTime
//
...
@interface WonderfulPicCollectionTableViewController ()
@property (nonatomic, strong) NSMutableArray *wonderfulArray;

@end

@implementation WonderfulPicCollectionTableViewController

- (NSMutableArray *)wonderfulArray{
    if (!_wonderfulArray) {
        _wonderfulArray = [[NSMutableArray alloc] init];
    }
...
- (void)setUpTableView{
    for (PictureModel *model  in [NSMutableArray arrayWithArray:[[FunnyTimeDataBase shareFunnyTimeDataBase] selectedAllPictureCollection]]) {
        if ([model.collectionType integerValue] == 2) {
...
//    headerView.titleLabel.text = @"精彩图片收藏";
...
//    self.tableView.tableHeaderView = headerView;
    ...
    self.navigationItem.title = @"精彩图片收藏";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PictureModel *model = self.wonderfulArray[indexPath.row];
